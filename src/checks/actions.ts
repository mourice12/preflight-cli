...
import { extractActionRefs } from '../parser';
import { checkActionExists, type ActionExistsResult } from '../github';

interface DeprecatedActionInfo {
  latest: string;
  versions: string[];
}

const DEPRECATED_ACTIONS: Record<string, DeprecatedActionInfo> = {
...
function splitActionRef(actionRef: string): { ownerRepo: string; ref: string } {
  const atIdx = actionRef.lastIndexOf('@');
  if (atIdx === -1) return { ownerRepo: actionRef, ref: '' };
...
export function makeActionsCheck(octokit: Octokit): CheckFunction {
  return {
    name: 'actions',
    ...
    async run({ workflows }) {
      ...
      for (const wf of workflows) {
        ...
        for (const ref of extractActionRefs(wf)) {
          occurrences.push({
      ...
      const existsCache = new Map<string, Promise<ActionExistsResult>>();
      for (const occ of occurrences) {
        if (!existsCache.has(occ.actionRef)) {
          existsCache.set(occ.actionRef, checkActionExists(octokit, occ.actionRef));
        }
      }

      for (const occ of occurrences) {
        const { ownerRepo, ref: version } = splitActionRef(occ.actionRef);
        const push = (r: Omit<CheckResult, 'file' | 'job' | 'step'>) => {
          const out: CheckResult = { ...r, file: occ.file, job: occ.job };
          if (occ.step) out.step = occ.step;
          results.push(out);
        };
