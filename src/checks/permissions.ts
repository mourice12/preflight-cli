import type { CheckFunction, CheckResult, WorkflowDefinition, JobDefinition } from '../types';
...
function attachJob(r: CheckResult, job?: string): CheckResult {
  if (job) r.job = job;
  return r;
}

function validatePermissionsBlock(
  perms: unknown,
  file: string,
  scopeLabel: string,
  job?: string,
  ...
  if (perms === undefined || perms === null) return results;
  ...
  for (const [scope, value] of Object.entries(perms as Record<string, unknown>)) {
    if (!VALID_SCOPES.has(scope)) {
      const suggestion = suggestTypo(scope, VALID_SCOPES);
      results.push(
        attachJob({
          check: 'permissions',
          severity: 'error',
...
function permsHasScope(perms: unknown, scope: string): boolean | 'write-all' {
...
function collectJobActions(job: JobDefinition): string[] {
  const out: string[] = [];
  for (const step of job.steps ?? []) {
    if (typeof step?.uses === 'string') out.push(step.uses.split('@')[0]);
  }
  return out;
}

export const permissionsCheck: CheckFunction = {
  name: 'permissions',
  ...
  async run({ workflows }) {
    ...
    for (const wf of workflows) {
      ...
      const doc: WorkflowDefinition = wf.parsed;

      results.push(
        ...validatePermissionsBlock(doc.permissions, wf.relativePath, 'at workflow level'),
      );
      ...
      for (const [jobName, job] of Object.entries(doc.jobs ?? {})) {
        ...
        if (activePerms === undefined) continue;
        ...
        if (
          uses.includes('actions/github-script') &&
          typeof activePerms === 'object' &&
          Object.keys(activePerms as object).length === 0
        ) {
          results.push({
            check: 'permissions',
            severity: 'info',
