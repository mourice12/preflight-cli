...
exports.runnersCheck = void 0;
const DEPRECATED_RUNNERS = {
...
function checkLabel(label, file, jobName, results) {
    ...
    if (trimmed.startsWith('self-hosted'))
        return;
    if (VALID_HOSTED.has(trimmed))
        return;
    results.push({
        check: 'runners',
        severity: 'info',
        ...
        for (const wf of workflows) {
            ...
            for (const [jobName, job] of Object.entries(jobs)) {
                ...
                if (typeof runsOn === 'string') {
                    ...
                    const strs = runsOn.filter((l) => typeof l === 'string');
                    if (strs.some((l) => l === 'self-hosted' || l.startsWith('self-hosted')))
                        continue;
