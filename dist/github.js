...
exports.checkActionExists = checkActionExists;
...
function getRepoInfo(cwd = process.cwd()) {
    let remoteUrl;
    try {
        remoteUrl = (0, node_child_process_1.execSync)('git remote get-url origin', {
            cwd,
        ...
        throw new Error(`Not a git repository or no "origin" remote configured (cwd: ${cwd}).\n` +
...
function parseGitHubRemote(remoteUrl) {
    ...
    if (gitProtoMatch)
        return { owner: gitProtoMatch[1], repo: gitProtoMatch[2] };
...
async function buildRepoContext(owner, repo, token) {
    ...
    const [repoRes, secretsRes, variablesRes, envsRes, orgSecretsRes, orgVarsRes] = await Promise.allSettled([
    ...
    const defaultBranch = repoRes.status === 'fulfilled' ? repoRes.value.data.default_branch : 'main';
    const secrets = new Set(['GITHUB_TOKEN']);
    if (secretsRes.status === 'fulfilled') {
        for (const s of secretsRes.value)
            secrets.add(s.name);
    }
    if (orgSecretsRes.status === 'fulfilled') {
        for (const s of orgSecretsRes.value)
            secrets.add(s.name);
    }
    const variables = new Set();
    if (variablesRes.status === 'fulfilled') {
        for (const v of variablesRes.value)
            variables.add(v.name);
    }
    if (orgVarsRes.status === 'fulfilled') {
        for (const v of orgVarsRes.value)
            variables.add(v.name);
    }
...
async function checkActionExists(octokit, actionRef) {
...
async function refExists(octokit, owner, repo, ref) {
    try {
